...
export default async function PaymentsPage() {
  ...
  const [schedule, services] = await Promise.all([
    ...
    }).then(rows => rows.map(s => ({ ...s, unitPrice: Number(s.unitPrice) }))),
  ]);

  return <PaymentsClient initialSchedule={schedule} initialServices={services} orgId={orgId} />;
}

async function fetchSchedule(orgId: string) {
  ...
  return tenants.map(tenant => {
    ...
    const rows: {
      type: 'DEPOSIT' | 'RENT'; period: string | null; monthIndex: number;
      dueDate: string; rentAmount: number; servicesTotal: number;
    ...
    while (cur <= currentPeriod) {
      ...
      const svcTotal = payment ? payment.services.reduce((s, c) => s + Number(c.amount), 0) : 0;
      rows.push({
        type: 'RENT', period: key, monthIndex,
        ...
        services:      payment ? payment.services.map(s => ({ name: s.service.name, units: s.units !== null ? Number(s.units) : null, amount: Number(s.amount) })) : [],
    ...
    return {
      id: tenant.id, name: tenant.name, unitId: tenant.unitId,
      unitName: tenant.unit.name, moveInDate: tenant.moveInDate.toISOString(),
      rentAmount, depositMonths: tenant.unit.depositMonths, depositAmount,
      rows, totalDue, totalPaid, totalBalance,
    };
  });
}
