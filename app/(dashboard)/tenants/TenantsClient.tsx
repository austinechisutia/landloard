...
interface HouseType { id: number; name: string; rentAmount: number }
interface Unit { id: number; name: string; status: 'VACANT' | 'OCCUPIED'; depositMonths: number; houseTypeId: number }
interface Tenant {
  id: number; name: string; phone: string; houseTypeId: number; unitId: number; moveInDate: string;
  idNumber: string | null; country: string | null; emergencyContact: string | null; emergencyPhone: string | null;
  householdCount: number; householdMembers: string | null;
  houseType: HouseType; unit: Unit;
}

interface BillingRow {
  type: 'DEPOSIT' | 'RENT'; period: string | null; monthIndex: number; dueDate: string;
  rentAmount: number; servicesTotal: number; services: { name: string; units: number | null; amount: number }[];
  amountDue: number; amountPaid: number; balance: number; status: string; paymentId: number | null; paymentDate: string | null;
}
interface TenantSchedule {
  id: number; name: string; unitName: string; moveInDate: string; rentAmount: number;
  depositMonths: number; depositAmount: number; rows: BillingRow[];
...
export default function TenantsClient({
  ...
  initialTypes: HouseType[];
}) {
  const router = useRouter();
  const [tenants,      setTenants]      = useState<Tenant[]>(initialTenants);
  const [types]                         = useState<HouseType[]>(initialTypes);
  ...
  const openAdd = () => { setEditing(null); setForm(blankForm); setUnits([]); setShowModal(true); };

  const openEdit = async (t: Tenant) => {
    setEditing(t);
    setForm({
      name: t.name, phone: t.phone, houseTypeId: String(t.houseTypeId), unitId: String(t.unitId),
      moveInDate: t.moveInDate.split('T')[0], idNumber: t.idNumber ?? '', country: t.country ?? '',
      emergencyContact: t.emergencyContact ?? '', emergencyPhone: t.emergencyPhone ?? '',
      householdCount: String(t.householdCount ?? 1), householdMembers: t.householdMembers ?? '',
    });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        name: form.name, phone: form.phone, houseTypeId: parseInt(form.houseTypeId),
        unitId: parseInt(form.unitId), moveInDate: form.moveInDate,
        idNumber: form.idNumber || null, country: form.country || null,
        emergencyContact: form.emergencyContact || null, emergencyPhone: form.emergencyPhone || null,
        householdCount: parseInt(form.householdCount) || 1, householdMembers: form.householdMembers || null,
      };
      if (editing) {
        const updated = await api.patch<Tenant>(`/tenants/${editing.id}`, payload);
        setTenants(prev => prev.map(t => t.id === editing.id ? { ...updated, payments: (t as Tenant & { payments?: unknown[] }).payments ?? [] } as Tenant : t));
        setToast({ msg: 'Tenant updated successfully', type: 'success' });
      } else {
        await api.post('/tenants', payload);
        // New tenant needs server-side data (payments etc) — trigger a refresh
        router.refresh();
        setToast({ msg: 'Tenant added successfully', type: 'success' });
      }
      setShowModal(false);
    } catch (err) {
  ...
  const parseMembers = (raw: string | null) =>
  ...
  return (
    <div className="space-y-6">
                ...
                {tenants.map(t => (
                  <tr key={t.id} className="hover:bg-gray-50">
                    <td className="px-6 py-3.5">
                      ...
                      {t.idNumber && <div className="text-xs text-gray-400">ID: {t.idNumber}</div>}
                    </td>
                    ...
                    <td className="px-6 py-3.5 text-gray-600">{new Date(t.moveInDate).toLocaleDateString()}</td>
                    <td className="px-6 py-3.5 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => setInfoTenant(t)} title="Client Details" className="p-1.5 text-sky-500 hover:text-sky-700 hover:bg-sky-50 rounded-lg transition-colors">
                        ...
                        <button onClick={() => openPaymentStatus(t)} title="Payment Status" className="p-1.5 text-emerald-600 hover:text-emerald-800 hover:bg-emerald-50 rounded-lg transition-colors">
        ...
        <Modal title={editing ? 'Edit Tenant' : 'Add Tenant'} onClose={() => setShowModal(false)}>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                  placeholder="e.g. John Mwangi" className="brand-input w-full rounded-lg border px-3 py-2 text-sm" required />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Phone Number</label>
                <input type="tel" value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))}
                  placeholder="e.g. 0712345678" className="brand-input w-full rounded-lg border px-3 py-2 text-sm" required />
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input type="text" value={form.idNumber} onChange={e => setForm(f => ({ ...f, idNumber: e.target.value }))}
                  placeholder="National ID / Passport" className="brand-input w-full rounded-lg border px-3 py-2 text-sm" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Country</label>
                <input type="text" value={form.country} onChange={e => setForm(f => ({ ...f, country: e.target.value }))}
                  placeholder="e.g. Kenya" className="brand-input w-full rounded-lg border px-3 py-2 text-sm" />
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <select value={form.houseTypeId} onChange={e => onTypeChange(e.target.value)}
                  ...
                  {types.map(t => <option key={t.id} value={t.id}>{t.name} — KSh {Number(t.rentAmount).toLocaleString()}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{editing ? 'Unit' : 'Available Unit'}</label>
                <select value={form.unitId} onChange={e => setForm(f => ({ ...f, unitId: e.target.value }))}
                  ...
                  disabled={!form.houseTypeId || loadingUnits} required>
                  <option value="">{!form.houseTypeId ? 'Select a house type first…' : loadingUnits ? 'Loading units…' : units.length === 0 ? 'No vacant units available' : 'Select a unit…'}</option>
                  {units.map(u => <option key={u.id} value={u.id}>{u.name}{editing && u.id === editing.unitId ? ' (current)' : ''}</option>)}
                </select>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input type="text" value={form.emergencyContact} onChange={e => setForm(f => ({ ...f, emergencyContact: e.target.value }))}
                  placeholder="e.g. Jane Mwangi" className="brand-input w-full rounded-lg border px-3 py-2 text-sm" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Emergency Contact Phone</label>
                <input type="tel" value={form.emergencyPhone} onChange={e => setForm(f => ({ ...f, emergencyPhone: e.target.value }))}
                  placeholder="e.g. 0722000000" className="brand-input w-full rounded-lg border px-3 py-2 text-sm" />
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <textarea value={form.householdMembers} onChange={e => setForm(f => ({ ...f, householdMembers: e.target.value }))}
          ...
          <div className="space-y-4 text-sm">
            <div>
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-2">
                <div><div className="text-xs text-gray-400">Full Name</div><div className="font-medium text-gray-900">{infoTenant.name}</div></div>
                <div><div className="text-xs text-gray-400">Phone</div><div className="text-gray-700">{infoTenant.phone}</div></div>
                <div><div className="text-xs text-gray-400">ID / Passport Number</div><div className="text-gray-700">{infoTenant.idNumber || <span className="text-gray-300">—</span>}</div></div>
                <div><div className="text-xs text-gray-400">Country</div><div className="text-gray-700">{infoTenant.country || <span className="text-gray-300">—</span>}</div></div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-2">
                <div><div className="text-xs text-gray-400">Unit</div><div className="font-semibold text-gray-900">{infoTenant.unit.name}</div></div>
                <div><div className="text-xs text-gray-400">House Type</div><div className="text-gray-700">{infoTenant.houseType.name}</div></div>
                <div><div className="text-xs text-gray-400">Move-in Date</div><div className="text-gray-700">{new Date(infoTenant.moveInDate).toLocaleDateString(undefined, { day: 'numeric', month: 'long', year: 'numeric' })}</div></div>
              </div>
            </div>
            <div className="border-t border-gray-100" />
            <div>
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-2">
                <div><div className="text-xs text-gray-400">Name</div><div className="text-gray-700">{infoTenant.emergencyContact || <span className="text-gray-300">—</span>}</div></div>
                <div><div className="text-xs text-gray-400">Phone</div><div className="text-gray-700">{infoTenant.emergencyPhone || <span className="text-gray-300">—</span>}</div></div>
              </div>
            </div>
            <div className="border-t border-gray-100" />
            <div>
              <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Household</h3>
              <div className="mb-2"><div className="text-xs text-gray-400">Number of People</div><div className="text-gray-700 font-medium">{infoTenant.householdCount}</div></div>
                  ...
                  <ul className="space-y-1">
                    {parseMembers(infoTenant.householdMembers).map((m, i) => (
                      <li key={i} className="flex items-center gap-2 text-gray-700">
                        <span className="w-5 h-5 rounded-full bg-indigo-100 text-indigo-700 text-xs flex items-center justify-center font-medium flex-shrink-0">{i + 1}</span>
                        {m}
                      </li>
                    ))}
                  </ul>
            ...
            <div className="pt-1">
              <button onClick={() => { setInfoTenant(null); openEdit(infoTenant); }}
      ...
      {paymentTenant && (
        <Modal title={`Payment Status — ${paymentTenant.name}`} onClose={() => { setPaymentTenant(null); setPaymentSchedule(null); }} size="lg">
          {paymentLoading ? (
              ...
              <div className="overflow-x-auto rounded-lg border border-gray-200">
                <table className="w-full text-xs">
                  <thead className="bg-gray-50 border-b border-gray-200">
                    <tr>
                      {['Type','Period','Rent','Services','Due','Paid','Balance','Due Date','Paid On','Status'].map(h => (
                        <th key={h} className={`px-3 py-2.5 text-gray-500 font-medium ${h === 'Rent' || h === 'Services' || h === 'Due' || h === 'Paid' || h === 'Balance' ? 'text-right' : h === 'Status' ? 'text-center' : 'text-left'}`}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                    {paymentSchedule.rows.map((row, i) => {
                      ...
                      const statusClass   = displayStatus === 'PAID' ? 'bg-green-100 text-green-700' : displayStatus === 'PARTIAL' ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700';
                      return (
                        <tr key={i} className={row.status === 'PENDING' && row.amountPaid === 0 ? 'bg-red-50/30' : 'hover:bg-gray-50'}>
                          <td className="px-3 py-2"><span className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-xs font-medium ${row.type === 'DEPOSIT' ? 'bg-violet-100 text-violet-700' : 'bg-blue-100 text-blue-700'}`}>{row.type === 'DEPOSIT' ? 'Deposit' : 'Rent'}</span></td>
                          <td className="px-3 py-2 text-gray-700">{row.type === 'RENT' && row.period ? <span>{fmtPeriod(row.period)} <span className="text-gray-400">#{row.monthIndex}</span></span> : <span className="text-gray-500">Move-in</span>}</td>
                          <td className="px-3 py-2 text-right text-gray-700">{row.rentAmount > 0 ? row.rentAmount.toLocaleString() : '—'}</td>
                          <td className="px-3 py-2 text-right text-gray-500">{row.servicesTotal > 0 ? <span title={row.services.map(s => `${s.name}: KSh ${s.amount.toLocaleString()}`).join('\n')} className="cursor-help underline decoration-dotted">{row.servicesTotal.toLocaleString()}</span> : '—'}</td>
                          ...
                          <td className={`px-3 py-2 text-right font-medium ${row.balance > 0 ? 'text-red-600' : 'text-green-600'}`}>{row.balance.toLocaleString()}</td>
                          <td className="px-3 py-2 text-gray-500">{new Date(row.dueDate).toLocaleDateString()}</td>
                          <td className="px-3 py-2 text-gray-500">{row.paymentDate ? new Date(row.paymentDate).toLocaleDateString(undefined, { day: 'numeric', month: 'short', year: 'numeric' }) : row.amountPaid > 0 ? <span className="text-amber-500">partial</span> : <span className="text-gray-300">—</span>}</td>
                          <td className="px-3 py-2 text-center"><span className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-xs font-medium ${statusClass}`}>{displayStatus === 'PARTIAL' ? 'Partial' : displayStatus.charAt(0) + displayStatus.slice(1).toLowerCase()}</span></td>
                        </tr>
                      );
                    })}
                  </tbody>
                  <tfoot className="border-t-2 border-gray-200 bg-gray-50">
                    <tr>
                      ...
                      <td className={`px-3 py-2.5 text-right font-semibold ${paymentSchedule.totalBalance > 0 ? 'text-red-600' : 'text-green-600'}`}>{paymentSchedule.totalBalance.toLocaleString()}</td>
                      <td colSpan={3} />
                    </tr>
                  </tfoot>
                </table>
      ...
      {deleteId !== null && (
        <Modal title="Remove Tenant" onClose={() => setDeleteId(null)}>
          ...
          <div className="flex gap-3">
            <button onClick={() => setDeleteId(null)} className="flex-1 border border-gray-300 text-gray-700 rounded-lg py-2 text-sm hover:bg-gray-50 transition-colors">Cancel</button>
            <button onClick={() => handleDelete(deleteId)} className="flex-1 bg-red-600 text-white rounded-lg py-2 text-sm hover:bg-red-700 transition-colors font-medium">Remove</button>
          </div>
        </Modal>
      )}
