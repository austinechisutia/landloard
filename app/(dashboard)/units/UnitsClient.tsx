...
interface Unit {
  id: number; name: string; status: 'VACANT' | 'OCCUPIED';
  depositMonths: number; houseTypeId: number; houseType: HouseType;
}
...
export default function UnitsClient({
  ...
  const openEdit = (u: Unit) => {
    setEditing(u);
    setForm({ name: u.name, houseTypeId: String(u.houseTypeId), depositMonths: String(u.depositMonths) });
    setShowModal(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) {
        const updated = await api.patch<Unit>(`/units/${editing.id}`, {
          name: form.name, depositMonths: parseInt(form.depositMonths),
        });
        setUnits(prev => prev.map(u => u.id === editing.id ? updated : u));
      } else {
        const created = await api.post<Unit>('/units', {
          name: form.name, houseTypeId: parseInt(form.houseTypeId), depositMonths: parseInt(form.depositMonths),
        });
                ...
                {units.map(u => {
                  ...
                  return (
                    <tr key={u.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-3.5 text-right font-medium text-gray-800">{depAmt.toLocaleString()}</td>
                      <td className="px-6 py-3.5">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${u.status === 'OCCUPIED' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                          {u.status === 'OCCUPIED' ? 'Occupied' : 'Vacant'}
                        </span>
                      </td>
                      <td className="px-6 py-3.5 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => openEdit(u)} title="Edit" className="brand-edit-btn rounded-lg p-1.5 transition-colors">
                            <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                          </button>
                          {u.status === 'VACANT' && (
                            <button onClick={() => setDeleteId(u.id)} title="Delete" className="p-1.5 text-red-500 hover:text-red-700 hover:bg-red-50 rounded-lg transition-colors">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {!editing && (
              <div>
                ...
                <select value={form.houseTypeId} onChange={e => setForm(f => ({ ...f, houseTypeId: e.target.value }))}
                ...
                {types.length === 0 && <p className="text-xs text-amber-600 mt-1">Add a house type first before adding units.</p>}
              </div>
            )}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Unit Name</label>
              <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                placeholder="e.g. A1, B2, 101" className="brand-input w-full rounded-lg border px-3 py-2 text-sm" required />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Deposit Required</label>
              <div className="flex gap-2 flex-wrap">
                {DEPOSIT_OPTIONS.map(n => (
                  <button key={n} type="button" onClick={() => setForm(f => ({ ...f, depositMonths: String(n) }))}
                    className={`px-3 py-1.5 rounded-lg text-sm font-medium border transition-colors ${form.depositMonths === String(n) ? 'bg-[#11430F] text-[#e4f386] border-[#11430F]' : 'border-gray-300 text-gray-700 hover:bg-gray-50'}`}>
                    {n} month{n !== 1 ? 's' : ''}
                  </button>
                ))}
              </div>
