...
function buildScheduleCacheFn(orgId: string, tenantId?: number) {
  return unstable_cache(
    async () => computeSchedule(orgId, tenantId),
    [`schedule:${orgId}:${tenantId ?? 'all'}`],
    { tags: [`schedule:${orgId}`] }
  );
}

async function computeSchedule(orgId: string, tenantId?: number) {
  const tenantIdFilter = tenantId ? { id: tenantId } : {};
...
export async function GET(request: Request) {
  try {
    ...
    const tenantId      = tenantIdParam ? parseInt(tenantIdParam) : undefined;
    const schedule = await buildScheduleCacheFn(orgId, tenantId)();
